    ...
    @if(auth()->check())
        <div class="topbar-actions" style="display: flex; align-items: center; gap: 12px;">
            ...
            <div class="topbar-notifications" style="position: relative;">
                <a href="{{ route('lpks.index', $alertCount > 0 ? ['surveillance' => 'NEEDS_ACTION'] : []) }}" title="{{ $alertCount > 0 ? $alertCount . ' Notifikasi Pengawasan Jatuh Tempo' : 'Tidak ada notifikasi aktif' }}" style="display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 8px; border: 1px solid #e2e8f0; background-color: {{ $alertCount > 0 ? '#fff1f2' : '#ffffff' }}; color: {{ $alertCount > 0 ? '#e11d48' : '#64748b' }}; position: relative; text-decoration: none;">
