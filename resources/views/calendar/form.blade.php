...
<form class="panel form-grid" method="POST" action="{{ $event->exists ? route('calendar.events.update', $event) : route('calendar.events.store') }}">
    ...
    <div class="form-actions full"><a class="button secondary" href="{{ route('calendar.index') }}">Batal</a><button class="button primary" type="submit">{{ $event->exists ? 'Simpan perubahan' : 'Simpan agenda' }}</button></div>
</form>
@endsection
