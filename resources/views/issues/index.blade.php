...
@section('title', 'Masalah | SIMASADI')

@section('content')
<x-page-header
    title="Laporan masalah"
    subtitle="Catat masalah operasional dan ikuti penyelesaiannya."
>
    <a class="button primary" href="{{ route('issues.create') }}">
...
<section class="panel">
    <form class="table-filters" method="GET">
        <div class="table-filter-grid">
            <label>
                Cari masalah
                <input name="search" value="{{ $search }}" placeholder="Judul masalah">
            </label>
            <label>
                LPK
                <select name="lpk_id">
            ...
            <label>
                Jatuh tempo dari
                <input type="date" name="due_from" value="{{ $dueFrom }}">
            </label>
            <label>
                Jatuh tempo sampai
        ...
        <div class="table-filter-actions">
            ...
            @if($search || $lpkId || $priority || $status || $dueFrom || $dueTo)
                <a class="button ghost" href="{{ route('issues.index') }}">Reset</a>
            @endif
        </div>
    </form>
                    ...
                    @foreach($issues as $issue)
                        <tr>
                            <td>
                                ...
                                <span>{{ $issue->due_date?->format('d M Y') ?: 'Tanpa target' }}</span>
                            </td>
                            <td>{{ $issue->lpk->name }}</td>
                            <td>
                                <span class="priority priority-{{ strtolower($issue->priority) }}">
                                    {{ ['LOW' => 'Rendah', 'MEDIUM' => 'Sedang', 'HIGH' => 'Tinggi'][$issue->priority] ?? $issue->priority }}
                                </span>
                            </td>
                            <td><x-status :value="$issue->status" /></td>
                            <td><a href="{{ route('issues.show', $issue) }}">Detail</a></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
    ...
    @else
        <div class="empty">
            {{ $search || $lpkId || $priority || $status || $dueFrom || $dueTo ? 'Tidak ada masalah yang cocok dengan filter.' : 'Belum ada laporan masalah.' }}
            @if($search || $lpkId || $priority || $status || $dueFrom || $dueTo)
                <a class="button ghost empty-action" href="{{ route('issues.index') }}">Reset filter</a>
            @endif
        </div>
    @endif
</section>
