...
<section class="panel">
    <form class="table-filters" method="GET">
        ...
        <div class="table-filter-actions"><button class="button secondary" type="submit">Terapkan filter</button>@if($lpkId || $status || $startFrom || $startTo || $targetFrom || $targetTo)<a class="button ghost" href="{{ route('accreditations.index') }}">Reset</a>@endif</div>
    </form>
    @if($accreditations->count())<div class="table-wrap"><table><thead><tr><th>LPK</th><th>Status</th><th>Mulai</th><th>Target</th><th></th></tr></thead><tbody>@foreach($accreditations as $item)<tr><td><strong>{{ $item->lpk->name }}</strong><span>{{ $item->lpk->registration_number }}</span></td><td><x-status :value="$item->status" /></td><td>{{ $item->start_date?->format('d M Y') ?: 'Belum diisi' }}</td><td>{{ $item->target_date?->format('d M Y') ?: 'Belum diisi' }}</td><td><a href="{{ route('accreditations.show',$item) }}">Detail</a></td></tr>@endforeach</tbody></table></div>{{ $accreditations->links() }}@else<div class="empty">{{ $lpkId || $status || $startFrom || $startTo || $targetFrom || $targetTo ? 'Tidak ada proses akreditasi yang cocok dengan filter.' : 'Belum ada proses akreditasi untuk ditampilkan.' }} @if($lpkId || $status || $startFrom || $startTo || $targetFrom || $targetTo)<a class="button ghost empty-action" href="{{ route('accreditations.index') }}">Reset filter</a>@endif</div>@endif
</section>
@endsection
