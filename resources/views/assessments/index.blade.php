...
<section class="panel">
    ...
    @if($assessments->count())<div class="table-wrap"><table><thead><tr><th>Agenda</th><th>LPK</th><th>Waktu</th><th>Status</th><th></th></tr></thead><tbody>@foreach($assessments as $assessment)<tr><td><strong>{{ $assessment->title }}</strong><span>{{ $assessment->assessment_type }}</span></td><td>{{ $assessment->lpk->name }}</td><td>{{ $assessment->start_at->format('d M Y, H:i') }}</td><td><x-status :value="$assessment->status" /></td><td><a href="{{ route('assessments.show',$assessment) }}">Detail</a></td></tr>@endforeach</tbody></table></div>{{ $assessments->links() }}@else<div class="empty">{{ $search || $lpkId || $assessmentType || $status || $startFrom || $startTo ? 'Tidak ada program asesmen yang cocok dengan filter.' : 'Belum ada program asesmen.' }} @if($search || $lpkId || $assessmentType || $status || $startFrom || $startTo)<a class="button ghost empty-action" href="{{ route('assessments.index') }}">Reset filter</a>@endif</div>@endif
</section>
@endsection
