    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin-bottom: 20px;">
        ...
        <div style="padding: 16px; background: var(--surface-subtle, #f8fafc); border: 1px solid var(--line); border-radius: 8px;">
            ...
            @if($assessment->tp_has_extension)
                ...
                @endif
            @else
                <div style="font-size: 14.5px; font-weight: 600; color: var(--muted); margin-bottom: 4px;">
                    Tidak ada
                </div>
                <small style="color: var(--muted); font-size: 12px; display: block; line-height: 1.4;">
                    Maksimal +1 bulan bersyarat jika ada progres perbaikan nyata
                </small>
            @endif
        </div>
            ...
            @else
                ...
                <small style="font-size: 12px; font-weight: 600; color: {{ $assessment->is_tp_overdue ? '#b91c1c' : ($assessment->days_remaining_tp <= 14 ? '#b45309' : '#1e40af') }}; display: block;">
                    ...
                    @elseif($assessment->days_remaining_tp <= 14)
                        Sisa {{ $assessment->days_remaining_tp ?? 0 }} hari (Mendekati Jatuh Tempo)
                    @else
